...
안녕하세요! 오늘은 드림핵(Dreamhack)의 **Simple Note Manager** 문제를 풀어보며, 수많은 삽질 끝에 깊게 공부하게 된  <font color='#4ADE80'>리눅스 쉘의 명령어 치환 기법</font>과 이를 제어하기 위한  <font color='#4ADE80'>백슬래시(`\`) 이스케이프</font>의 핵심 원리를 공유해 보려고 합니다.
...
이 문제를 풀 때 가장 많은 고전과 시행착오를 겪게 만드는 주범은 바로 " <font color='#4ADE80'>명령어 치환</font>" 메커니즘입니다. <br>
...
이전 글과 같이 이러한 보안 문제를 막기 위해서는 가장 먼저 할 수 있는 생각이 shell=False였습니다.
...
def backup_notes(timestamp):
    with lock:
        # 쓰기 모드로 파일 실행
        with open('./tmp/notes.tmp', 'w') as f:
            shutil.copyfile('./tmp/notes.tmp', f'/tmp/{timestamp}', follow_symlinks=False)
```
외부 프로그램(cp)을 빌려 쓰지 말고, 파이썬의 shutil 라이브러리를 사용하면 코드를 딱 한 줄만으로 안전하게 바꿀 수 있습니다.<br>
저번 글에서 `subprocess.run()` 함수를 리스트 형식으로 변경한 후 shell=False로 지정해주었으니 이번에는 새로운 방법을 찾아 구상해보았습니다.<br>

<font color='#4ADE80'>shutil.copy</font>는 내부적으로 리눅스 쉘(/bin/sh)을 아예 실행하지 않고 시스템 콜을 직접 호출합니다.<br>

우리가 앞서 성공했던 공격 코드 속의 $( )이나 백틱(`) 기호들은 리눅스 쉘이라는 환경이 존재해야만 명령어로 해석되어 작동하는 특수 기호들입니다.<br>

하지만 위와 같이 파이썬 순수 코드로 대체해 버리면, 해커가 쿠키 값에 공격 기호를 아무리 섞어 보내도 쉘이 켜지지 않으므로 기호들이 터지지 않습니다.<br> 서버는 그저 $(curl...)이라는 기호가 통째로 포함된 문자열을 '단순 파일 이름'으로 취급할 뿐입니다. 즉, 명령어 주입(Command Injection) 취약점이 원천 봉쇄됩니다.

그러나 단순 shutil.copy는 타깃 경로에 공격자가 심어둔  <font color='#ff3b0a'>심볼릭 링크</font>가 있을 경우, 링크가 가리키는 원본 대상을 쫓아가서(Follow) 내용을 덮어써 버리는 취약점이 존재합니다.<br>

따라서 공격자가 /tmp/{timestamp} 경로를 가로채 시스템 중요 파일로 링크를 걸어두면 파일이 훼손될 위험이 있습니다. <br>
이를 막기 위해  <font color='#4ADE80'>follow_symlinks=False</font> 옵션을 제공하는 shutil.copyfile을 사용하는 것이 가장 안전합니다.
...
최근 다양한 워게임 문제들을 분석하다 보니, 흥미롭게도 연속해서 Command Injection 취약점을 접하게 되었습니다.<br> 의도하고 고른 것은 아니었지만, 소스코드를 분석하는 과정에서 자연스럽게 취약점을 식별하고 익스플로잇하는 재미를 알아가는 중입니다.<br>

특히 이번 문제는 명령어 주입 자체에 그치지 않고, 이를 위한 스크립트를 직접 구성해 보면서 리눅스 쉘(Shell)의 내부 동작 특징과 까다로운 이스케이프 문법을 깊이 있게 이해할 수 있는 정말 좋은 기회였습니다.<br>

**한 걸음씩 화이트해커와 시큐어 코더의 시선을 동시에 넓혀가는 중입니다.<br> 제 풀이와 시큐어 코딩 구현 과정이 도움이 되셨다면 공감과 댓글 부탁드립니다. 피드백은 언제나 환영합니다!**
