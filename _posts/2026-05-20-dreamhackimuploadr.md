...
categories: [wargame,시그니처,signature,서버]
toc : true
toc_sticky: true
toc_label: "Contents"
author_profile: true
...
-이 파일이  **<font color='Cyan'>특정한 확장자 명을 가졌다는 것을 알려주는 hex값!</font>** 입니다.
...
-파일의 **<font color='Cyan'> 가장 마지막 부분에 위치하는 바이트 패턴 값</font>**(흔히 Tralier라고 부릅니다.)
 ...
 푸터 시그니처로 49 45 4E 44 AE 42 60 82 를 가집니다.
...
{: .no_toc}
확장자와 MIME타입을 검사할 때 사용되는 &&(and)연산자를 **<font color='Lime'>||(OR)연산자로 변경</font>**하거나, **<font color='Lime'>각각 독립된 if문으로 분리</font>**하여 둘 중 하나라도 화이트리스트 규칙을 만족하지 못하면 무조건 업로드를 차단하도록 **<font color='Lime'>로직의 Suri</font>**가 필요합니다!
...
이렇게 **<font color='Lime'>개별 검증 패턴</font>**으로 로직을 분리하여 확장자가 .php이거나 MIME 타입이 조작된 그 어떤 파일이 들어오더라도 각각의 방어벽에 가로막혀 업로드가 원천 차단된 것을 확인 할 수 있습니다!


### 5-2 : 업로드 디렉터리의 웹 스크립트 실행 권한 제거
{: .no_toc}

소스코드를 수정하는 것 외에도, 웹 서버 환경에서 2중 방어벽을 세우는 **<font color='Lime'> '심층 방어'</font>**에 대해서도 이야기 해보려고 합니다.
...
```
이렇게 구성하면 AddType 지시어로 인해 PHP 확장자들이 정의되더라도, 하단의 **FilesMatch** 블록이 해당 확장자를 가진 우회 업로드한 스크립트 엔진이 실행 되지 않도록 막아 줄 수 있습니다!
...
<div style="text-align: center; margin: 20px 0;">
  <img src="{{ '/images/imageuploader/server.png' | relative_url }}"
       alt="심층 방어"
       style="max-width: 80%; height: auto; border: 1px solid #ddd; border-radius: 5px;">
  <p style="font-size: 0.9em; color: #ffffff;">[서버설정을 활용한 심층 방어 성공!]</p>
</div>
...
이번 Dreamhack **Image Uploader** 문제는 단순한 필터링 코드 한 줄, 혹은 논리 연산자 하나를 잘못 선택하는 것이 시스템 전체의 권한을 넘겨 줄 수 있게 만드는 취약점임을 상기시켜주는 문제였습니다.

특히 컴퓨터가 파일 형식을 판별할 때 성능 효율성을 위해 **'<font color='Lime'>특정 위치의 고유 매직 바이트 시퀀스</font>'**만 검사한다는 메커니즘을 정확히 이해하고 있어야만, 이러한 교묘한 우회 공격을 진단하고 완벽하게 방어할 수 있다는 것에 대해 다시 한번 공부 할 수 있었습니다.
