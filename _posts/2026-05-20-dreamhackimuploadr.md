...
특히 컴퓨터가 파일 형식을 판별할 때 성능 효율성을 위해 **'특정 위치의 고유 매직 바이트 시퀀스'**만 검사한다는 메커니즘을 정확히 이해하고 있어야만, 이러한 교묘한 우회 공격을 진단하고 완벽하게 방어할 수 있다는 것에 대해 다시 한번 공부 할 수 있었습니다.

추가로, 이전에 해결했던 **Really NOT SQL** 문제를 다시 한번 복습하며 본 문제에 대조 및 접목해 본 과정이 매우 뜻깊었습니다.  
인프라 단에서의 서버 설정이 안전하게 이루어져야 비로소 완벽한 방어가 완성된다는 시야를 넓힐 수 있었던 유익한 실습이었습니다.

이것으로 Image Uploader 문제 분석을 마치며, 서버 설정의 허점을 더 깊게 파고들었던 `Really NOT SQL` 문제의 흥미진진한 가설 설정과 풀이 과정은 추후 포스팅에서 더 자세히 다루어 보도록 하겠습니다!
